...
,{heading:'💡 Real-World: F-35 JSF — MBSE Governance on a $400 Billion Program',paragraphs:['The Lockheed Martin F-35 program manages over 150,000 model elements in IBM Rational Rhapsody with Teamwork Cloud. A formal Model Governance Plan assigns package ownership by engineering discipline — no engineer writes to a package they do not own without an approved Engineering Change Proposal. Automated quality gate checks run every 24 hours: orphan detection (zero tolerance), naming convention compliance (100%), traceability coverage (≥95%). Any failure generates a Problem Report with a 48-hour resolution window.','Why does this matter for a graduate student? Because the habits you form in this course are the habits that a program manager at Lockheed, Boeing, or Northrop Grumman will evaluate in your first performance review. "Did the engineer build a queryable, traceable model that other team members can read and extend?" or "Did the engineer build a diagram that only they understand?" Professional MBSE is a team discipline, not an individual art form. The naming convention, the package architecture, the quality gate discipline — these are professional skills as important as the SysML syntax itself.'],callout:{type:'gold',label:'MagicGrid 3×3 Matrix — Which Cell Are You In Right Now?',body:'Row 1 (Stakeholder): Structural = context use case diagram. Behavioral = operational scenarios. Analytical = concept-level trade studies. Row 2 (System/Black-Box): Structural = system-level BDD. Behavioral = use case diagram, activity diagram. Analytical = system-level parametric. Row 3 (Component/White-Box): Structural = component BDDs and IBDs. Behavioral = sequence diagrams, state machines. Analytical = component parametric models. Rule: complete left-to-right before moving top-to-bottom. Never build component models before system architecture is established.'}},{heading:'💡 Real-World: Boeing 777X Folding Wingtip — Digital Thread in Production',paragraphs:['Boeing\'s 777X folding wingtip mechanism involves 47 actuators, 23 sensors, and 8 interlock conditions. Boeing used a single authoritative SysML state machine as the source for the flight control software specification, the structural test procedure, the maintenance manual procedure, and the FAA certification test plan. Any discrepancy between implementation and the SysML model was treated as a non-conformance requiring formal change approval.','This eliminated the "document drift" failure mode — where four documents describing the same system gradually diverge through separate revision cycles until no one knows which is authoritative. In a document-centric program, the maintenance manual for the wingtip mechanism might still reference step 7 as "verify actuator position" while the updated software specification changed the verification to a torque sensor check in revision C of the ICD — and the maintenance crew is following an outdated procedure. In the SysML-driven digital thread, there is only one source of truth. The maintenance procedure is generated from the same state machine that generated the software specification.']}],
    schemaVisualizer: [
      {
